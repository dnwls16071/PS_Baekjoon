...
first_alphabet = [0] * 26
second_alphabet = [0] * 26

first = input().strip()
second = input().strip()

for i in first:
    first_alphabet[ord(i)-97] += 1

for i in second:
    second_alphabet[ord(i)-97] += 1

deleted_cnt = 0
for i in range(26):
    if first_alphabet[i] == second_alphabet[i]:
        continue
    else:
        deleted_cnt += abs(first_alphabet[i] - second_alphabet[i])
print(deleted_cnt)